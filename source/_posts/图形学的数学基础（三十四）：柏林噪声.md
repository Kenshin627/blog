...
![二维极坐标系](图形学的数学基础（三十四）：柏林噪声/2.jpg)

我们知道球体的球面度（立体角）为$4\pi$,$PDF$（概率密度函数）的积分为1，因此，球体微分立体角的概率为$\dfrac{1}{4\pi}$：
...
至此我们拿到了$\phi和\theta$的$PDF$,通过$PDF$可以很轻易的计算$CDF$（累积分布函数）：
...
其中希腊字母$\xi$表示[0,1]范围内均匀分布的随机数。也就是说要在球体表面上生成均匀的随机点，需要对代码做以下优化：
...
![二维极坐标系](图形学的数学基础（三十四）：柏林噪声/3.jpg)
...
![二维极坐标系](图形学的数学基础（三十四）：柏林噪声/4.jpg)
